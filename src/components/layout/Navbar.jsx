...
const Navbar = ({ onOpenChat }) => {
    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md border-b border-zinc-200 dark:border-white/5' : 'bg-transparent'}`}>
            <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                <div className="flex items-center gap-2 text-xl font-bold tracking-tighter text-zinc-900 dark:text-white">
                ...
                <div className="hidden md:flex items-center gap-8">
                    {navLinks.map((link) => (
                        <a
                            ...
                            target={link.target}
                            rel={link.rel}
                            className="text-sm font-medium text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white transition-colors relative group"
                        >
                            {link.name}
                    ...
                    <button
                        onClick={onOpenChat}
                        className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/5 dark:bg-white/5 border border-zinc-200 dark:border-white/10 hover:border-core-red/50 hover:bg-core-red/10 transition-all duration-300 group"
                    >
                    ...
                    <div className="flex items-center gap-4 pl-4 border-l border-zinc-200 dark:border-white/10">
                        <button onClick={toggleTheme} className="text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors">
                            {theme === 'light' ? <Moon size={20} /> : <Sun size={20} />}
                        </button>
                        <a href="https://github.com/sahilbagde22" target="_blank" rel="noreferrer" className="text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors"><Github size={20} /></a>
                        <a href="https://www.linkedin.com/in/sahilbagade/" target="_blank" rel="noreferrer" className="text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors"><Linkedin size={20} /></a>
                    </div>
                </div>
